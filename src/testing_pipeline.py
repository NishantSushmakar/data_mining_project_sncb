...
def main():
    ...
    # Chaining the transformers manually for now - figuring out how to avoid the .fit() pipeline restrictions
    df = ConvertSequencesToLists(sequence_columns=sequence_columns).fit(df)
    df = OutlierImputer(lat_extreme=[49.5072, 51.4978], lon_extreme=[2.5833, 6.3667], n_neighbors=2).fit(df)
    df = AddIndexSequence().fit(df)
    df = AddWindowIndices(window_start=3600, window_end=600).fit(df)
    df = AddWindowedSequences(columns=sequence_columns).fit(df)


    df = UniqueAndHighestFrequencyPercentageSequences().fit(df)
